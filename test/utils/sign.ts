...
/**
 * Getter for a signer
 * @param index index to get in signers array
 * @returns the signer
 */
export async function getSignerAt(
  index: number = 0
...
/**
 * Getter for the owner, default address
 * @returns the signer
 */
export async function getOwner(): Promise<SignerWithAddress> {
  return getSignerAt(0)
}
 ...
 * @returns the signature
 */
export async function signClaimTypeMessage(
  socialID: string,
  recipient: SignerWithAddress,
  feeAmount: number,
  deadline: number,
  ...
  const typeDataApprove = claimTypeMessage(
    socialID,
    recipient,
    feeAmount,
    deadline,
...
/**
 * Creates the typed message for signing for the claimTokensOnBehalf contract call
 *
 * @param socialID the social id of the claim
 * @param recipient the recipient of the claimed funds
