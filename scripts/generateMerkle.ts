...
const WEICO_CONSTANT = 18

// the posible prefixes for the two social networks' ids
type IDPrefix = "discord:" | "twitter:"
...
;(async function () {
  ...
  const data = await Promise.all([
    await loadPointsData(
      path.join(__dirname, "/input/discord-points-final.csv"),
      "discord:"
    ),
    await loadPointsData(
      path.join(__dirname, "/input/twitter-points-final.csv"),
      "twitter:"
    ),
  ])
 ...
 * @returns
 */
function generateTree(claims: ClaimElement[]): ClaimsMerkleTree {
 ...
 * @returns
 */
function weicofyPoint(rawPoint: string): string {
